...
import { authenticate } from './modules/utils.js';
        ...
        <div id="barkFields" class="hidden">
          ...
          <div class="form-group">
            ...
            <div class="sound-options">
              ...
              <div class="sound-option" data-sound="bells" onclick="selectSound(this)">Bells</div>
              <div class="sound-option" data-sound="california" onclick="selectSound(this)">California</div>
              <div class="sound-option" data-sound="chime" onclick="selectSound(this)">Chime</div>
              <div class="sound-option" data-sound="glass" onclick="selectSound(this)">Glass</div>
              <div class="sound-option" data-sound="notification" onclick="selectSound(this)">Notification</div>
            </div>
          </div>
        </div>
...
async function getPushConfigs(userId, env) {
  try {
    const ADMIN_USERNAME = env.ADMIN_USERNAME || '';
    let data;
    ...
    const safeConfigs = configs.map(c => {
      const safeConfig = { id: c.id, name: c.name, type: c.type, config: {} };
      if (c.type === 'bark') {
        safeConfig.config.hasUrl = true;
        if (c.config.sound) safeConfig.config.sound = c.config.sound;
      } else if (c.type === 'serverchan') {
        safeConfig.config.hasKey = true;
      }
      return safeConfig;
    });
...
async function addPushConfig(request, userId, env) {
  try {
    const body = await request.json();
    ...
    if (!type || !['bark', 'serverchan'].includes(type)) {
      return sendJSON({ error: '推送类型不合法' }, 400);
    }
    
    if (type === 'bark') {
      if (!config.url || !config.url.startsWith('http')) {
        return sendJSON({ error: 'Bark URL 不合法' }, 400);
      }
    } else if (type === 'serverchan') {
      if (!config.sendKey || config.sendKey.length < 5) {
        return sendJSON({ error: 'SendKey 不合法' }, 400);
      }
    }
...
async function deletePushConfig(request, userId, env) {
  try {
    const url = new URL(request.url);
...
async function testPushConfig(request, userId, env) {
  try {
    const body = await request.json();
    const { id } = body;
...
async function testNewPushConfig(request, userId, env) {
  try {
    const body = await request.json();
...
async function sendPush(pushConfig, title, content) {
  try {
    if (pushConfig.type === 'bark') {
      return await sendBark(pushConfig.config, title, content);
    } else if (pushConfig.type === 'serverchan') {
      return await sendServerChan(pushConfig.config, title, content);
    }
...
async function sendBark(config, title, content) {
...
async function getReminders(userId, env) {
  try {
    const ADMIN_USERNAME = env.ADMIN_USERNAME || '';
...
async function addReminder(request, userId, env) {
  try {
    const body = await request.json();
...
async function deleteReminder(request, userId, env) {
  try {
    const url = new URL(request.url);
    const id = url.searchParams.get('id');
    if (!id) {
...
async function testCheckReminders(userId, env) {
...
async function getAllUsers(request, adminUserId, env) {
  try {
    if (!await isAdmin(adminUserId, env)) {
...
async function deleteUser(request, adminUserId, env) {
  try {
    if (!await isAdmin(adminUserId, env)) {
      return sendJSON({ error: '权限不足' }, 403);
    }
...
<body>
    ...
    <div id="mainSection" class="hidden">
      ...
      <div class="card fade-in">
        ...
        <div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid rgba(255,255,255,0.1);">
          <button class="btn btn-secondary btn-small" onclick="testCheckReminders()">测试每日检查</button>
        </div>
      </div>
      ...
      <form onsubmit="handleAddPushConfig(event)">
        ...
        <div id="barkFields" class="hidden">
          <div class="form-group">
            <label>Bark 服务器地址</label>
            <input type="url" id="barkUrl" placeholder="例如: https://api.day.app/ABC123">
            <p style="color: rgba(255,255,255,0.4); font-size: 0.85em; margin-top: 5px;">在 Bark App 中获取设备 Key</p>
          </div>
          <div class="form-group">
            <label>提示音 (可选)</label>
            <div class="sound-options">
  ...
  <script>
    ...
    function handleRegister(e) {
      ...
      ajaxRequest("/api/auth/register", "POST", 
        ...
        function(err) {
          ...
          isLoading = false;
          if (btn) { btn.textContent = "注册"; btn.classList.remove("loading"); }
          showMessage("authError", "注册失败，请稍后重试");
        },
        10000
      );
    }
    ...
    function showAddPushConfig() {
      ...
      document.getElementById("barkUrl").value = "";
      document.getElementById("serverchanKey").value = "";
      document.querySelectorAll(".sound-option").forEach(function(el) { el.classList.remove("selected"); });
      selectedSound = "";
      updatePushConfigFields();
      document.querySelectorAll("#pushConfigError, #pushConfigSuccess").forEach(function(el) { el.classList.add("hidden"); });
      document.getElementById("pushConfigModal").classList.remove("hidden");
      document.getElementById("modalOverlay").classList.remove("hidden");
    }
    ...
    function selectSound(el) {
      document.querySelectorAll(".sound-option").forEach(function(e) { e.classList.remove("selected"); });
      el.classList.add("selected");
      selectedSound = el.getAttribute("data-sound");
    }
